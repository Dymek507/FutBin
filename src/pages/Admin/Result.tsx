...
const Result = ({ resultData }: ResultProps) => {
  ...
  const { resultId, date, userOneUid, userTwoUid, userOneGoals, userTwoGoals } =
    resultData;
  ...
  return (
    <>
          ...
          <div className="flex w-full gap-4 px-4">
            <div className="flex flex-col items-center grow gap-4 pt-2 bg-sky-800 h-72 w-72">
              ...
              <Typography variant="h4">
                +{resultHandler(userOneGoals, userTwoGoals)}
              </Typography>
            </div>
            <div className="flex flex-col items-center grow gap-4 pt-2 bg-sky-800 h-72 w-72">
              ...
              <Typography variant="h4">
                +{resultHandler(userTwoGoals, userOneGoals)}
              </Typography>
            </div>
          </div>
        </div>
      ...
      <div
        ...
        className="flex-center px-4 h-12 text-2xl bg-gradient-to-r from-cyan-500 to-blue-500 rounded-xl shadow-xl"
      >
        <div className="flex flex-row">
          <p className="ml-2">{userOneData.login}</p>
          <p className="mx-2">{userOneGoals}</p>:
          <p className="mx-2">{userTwoGoals}</p>
          <p className="mr-2">{userTwoData.login}</p>
        </div>
      </div>
    </>
  );
